...
import { ProductCardsPreview } from "@/components/commerce/product-cards-preview";
import { isProductMessagePayload, type OrderPayload } from "@/lib/commerce/types";
import { orderTotal } from "@/lib/commerce/catalog";
...
function MessageContent({
  ...
  switch (message.content_type) {
    ...
    case "interactive": {
      ...
      if (isProductMessagePayload(message.interactive_payload)) {
        return <ProductCardsPreview payload={message.interactive_payload} />;
      }
    ...
    case "order":
      if (message.order_payload) {
        return <OrderContent order={message.order_payload} t={t} />;
      }
      return (
        <p className="whitespace-pre-wrap break-words text-sm">
          {message.content_text || t("order")}
        </p>
      );
...
/** A WhatsApp cart the customer sent from our catalog product cards. */
    ...
    <div className="flex min-w-[200px] flex-col gap-1 text-sm">
      ...
      <ul className="flex flex-col gap-0.5">
        {order.items.map((item, i) => (
          <li key={`${item.retailer_id}-${i}`} className="flex justify-between gap-3">
            <span className="min-w-0 break-words">
              {item.quantity} × {item.name || item.retailer_id || t("orderUnknownItem")}
            </span>
            {item.item_price != null ? (
              <span className="shrink-0 tabular-nums">
                {formatAmount(item.item_price * item.quantity, item.currency)}
              </span>
            ) : null}
          </li>
        ))}
      ...
      {order.text ? (
        <p className="whitespace-pre-wrap break-words text-xs opacity-80">{order.text}</p>
      ) : null}
    </div>
  );
}
