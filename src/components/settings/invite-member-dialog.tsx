...
export function InviteMemberDialog({
  ...
  async function handleCreate() {
    ...
    try {
      ...
      setResult({
        ...
        accountName: account?.name ?? 'our SKWEE Connect account',
      });
      onCreated();
    } catch (err) {
  ...
  function whatsappShareUrl(url: string): string {
    ...
    // for users in multi-team contexts where "our SKWEE Connect
    // account" wouldn't be enough to disambiguate.
    const accountName = result?.accountName ?? 'our SKWEE Connect account';
