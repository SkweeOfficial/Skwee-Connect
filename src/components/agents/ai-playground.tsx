...
import {
  isProductMessagePayload,
  type ProductMessagePayload,
...
interface Turn {
  ...
  handoff?: boolean;
  /** assistant-only: the product cards the bot would send on WhatsApp. */
  productCards?: ProductMessagePayload | null;
}

/** What the model sees for a turn — cards are named, as in the live bot. */
function transcriptText(t: Turn): string {
  if (!t.productCards) return t.content;
  const note = productCardsNote(t.productCards);
  ...
  const send = async () => {
    ...
    try {
      const res = await fetch('/api/ai/playground', {
        ...
        body: JSON.stringify({
          messages: next.map((t) => ({ role: t.role, content: transcriptText(t) })),
        }),
      });
      ...
      setTurns([
        ...next,
        {
          ...
          productCards: isProductMessagePayload(data.product_cards)
            ? data.product_cards
            : null,
        },
      ]);
      ...
      <div ref={scrollRef} className="flex-1 space-y-4 overflow-y-auto p-4">
        {turns.length === 0 && (
          <div className="flex h-full flex-col items-center justify-center text-center text-sm text-muted-foreground">
            ...
            <p className="mt-1 text-xs">
              It uses your knowledge base and product catalog and behaves
              exactly like the auto-reply bot — including product cards and
              handoff.
            </p>
            {onGoToSetup && (
              <Button
        ...
        {turns.map((t, i) => (
          ...
          >
            ...
            >
              {t.productCards ? (
                // The card carries the reply as its body, as on WhatsApp.
                <ProductCardsPreview payload={t.productCards} />
              ) : (
