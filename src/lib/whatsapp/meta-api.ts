...
// ============================================================
// Commerce (catalog product cards)
// ============================================================
//
// Product messages render a card from the Meta Commerce catalog linked
// to the sending number: image, name and price come from the catalog,
// and the customer can open it, add to the WhatsApp cart and send an
// order without leaving the chat. The order comes back as an inbound
// `order` message (see the webhook).
//
//   https://developers.facebook.com/docs/whatsapp/cloud-api/guides/sell-products-and-services/share-products

/** Meta limits for product / product-list messages. */
...
export interface SendProductMessageArgs {
  ...
  to: string
  catalogId: string
  /** The catalog item's retailer id (the feed's `id` column). */
  productRetailerId: string
  /** Optional text above the card (≤ 1024 chars). */
  bodyText?: string
  footerText?: string
}
...
export async function sendProductMessage(
  ...
  const {
    phoneNumberId, accessToken, to,
    catalogId, productRetailerId, bodyText, footerText,
  } = args
  if (!catalogId) throw new Error('Product message requires a catalogId.')
  if (!productRetailerId) {
    throw new Error('Product message requires a productRetailerId.')
  }
  validateProductText(undefined, bodyText, footerText)
...
export interface ProductListSection {
  /** Section heading (≤ 24 chars), e.g. the product name. */
  title: string
...
export interface SendProductListMessageArgs {
  ...
  to: string
  catalogId: string
  /** Required by Meta for product lists (≤ 60 chars). */
  headerText: string
  /** Required by Meta for product lists (≤ 1024 chars). */
  bodyText: string
  footerText?: string
...
/** Send a multi-product message (a scrollable list of catalog items). */
export async function sendProductListMessage(
  args: SendProductListMessageArgs
): Promise<MetaSendResult> {
  const {
    phoneNumberId, accessToken, to,
    catalogId, headerText, bodyText, footerText, sections,
  } = args
  ...
  if (!bodyText) throw new Error('Product list requires bodyText.')
  validateProductText(headerText, bodyText, footerText)
  ...
  for (const s of sections) {
    if (!s.title) throw new Error('Product list section missing title.')
    if (s.title.length > PRODUCT_MESSAGE_LIMITS.sectionTitleMaxLength) {
      throw new Error(
        `Product list section title "${s.title}" exceeds ${PRODUCT_MESSAGE_LIMITS.sectionTitleMaxLength} chars.`
      )
    }
    if (s.productRetailerIds.length < 1) {
      throw new Error(`Product list section "${s.title}" has no products.`)
    }
    for (const id of s.productRetailerIds) {
      if (!id) throw new Error('Product list item missing retailer id.')
      if (seen.has(id)) {
        throw new Error(`Product list has duplicate product "${id}".`)
      }
      seen.add(id)
    }
  }
  if (seen.size > PRODUCT_MESSAGE_LIMITS.maxProductsTotal) {
    throw new Error(
      `Product list allows at most ${PRODUCT_MESSAGE_LIMITS.maxProductsTotal} products (got ${seen.size}).`
    )
  }
...
function validateProductText(
  ...
  if (headerText && headerText.length > PRODUCT_MESSAGE_LIMITS.headerTextMaxLength) {
    throw new Error(
      `Product message headerText exceeds ${PRODUCT_MESSAGE_LIMITS.headerTextMaxLength} chars.`
    )
  }
  if (bodyText && bodyText.length > PRODUCT_MESSAGE_LIMITS.bodyMaxLength) {
    throw new Error(
      `Product message bodyText exceeds ${PRODUCT_MESSAGE_LIMITS.bodyMaxLength} chars.`
    )
  }
  if (footerText && footerText.length > PRODUCT_MESSAGE_LIMITS.footerMaxLength) {
    throw new Error(
      `Product message footerText exceeds ${PRODUCT_MESSAGE_LIMITS.footerMaxLength} chars.`
    )
  }
}
...
/** One catalog item as the Graph API returns it. */
...
export interface ListCatalogProductsArgs {
  catalogId: string
  accessToken: string
  /** Safety cap on pages fetched (100 items each). */
  maxPages?: number
}
...
/**
 * Read every item in a Meta Commerce catalog, following the Graph API's
 * cursor paging. The access token's system user needs the
 * `catalog_management` permission and access to the catalog.
 */
export async function listCatalogProducts(
  args: ListCatalogProductsArgs
): Promise<MetaCatalogProduct[]> {
  const { catalogId, accessToken, maxPages = 50 } = args
  if (!catalogId) throw new Error('listCatalogProducts requires a catalogId.')

  const out: MetaCatalogProduct[] = []
  let next: string | null =
    `${META_API_BASE}/${encodeURIComponent(catalogId)}/products` +
    `?fields=${CATALOG_PRODUCT_FIELDS}&limit=100`
  for (let page = 0; next && page < maxPages; page++) {
    ...
    for (const item of data.data ?? []) {
      if (item?.retailer_id && item?.name) out.push(item)
    }
