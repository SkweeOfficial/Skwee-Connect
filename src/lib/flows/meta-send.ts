...
import { productPayloadPreviewText } from '@/lib/commerce/catalog'
...
interface SendProductsEngineArgs {
  accountId: string
  userId: string
  conversationId: string
  contactId: string
  /** Built by `buildProductMessagePayload` — also what gets persisted. */
  payload: ProductMessagePayload
  /** Marks the persisted row as an AI reply (inbox "AI" badge). */
  aiGenerated?: boolean
}

/**
 * Send catalog product cards — a single product or a product list —
 * and persist the message with its payload so the inbox can render the
 * cards. Same phone-variant retry + persistence as the senders above.
 */
export async function engineSendProducts(
  args: SendProductsEngineArgs,
): Promise<{ whatsapp_message_id: string }> {
  const db = supabaseAdmin()
  const { payload } = args
  ...
  const attempt = async (phone: string): Promise<string> => {
    if (payload.kind === 'product') {
      const r = await sendProductMessage({
        ...
        productRetailerId: payload.sections[0]?.items[0]?.retailer_id ?? '',
    ...
    const r = await sendProductListMessage({
      ...
      sections: payload.sections.map((s) => ({
        title: s.title,
        productRetailerIds: s.items.map((i) => i.retailer_id),
      })),
    })
    return r.messageId
  }
  ...
  const preview = productPayloadPreviewText(payload)
  const { error: msgErr } = await db.from('messages').insert({
    conversation_id: args.conversationId,
    sender_type: 'bot',
    content_type: 'interactive',
