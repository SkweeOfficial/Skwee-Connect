...
describe('parseGeneration', () => {
  ...
  it('collects + strips product markers', () => {
    expect(
      parseGeneration(
        'These would suit her:\n\n[[PRODUCTS: dress-1, tee-2 ,dress-1]]',
      ),
    ).toEqual({
      text: 'These would suit her:',
      handoff: false,
      productIds: ['dress-1', 'tee-2'],
      usage: null,
    })
  })

  it('returns empty text when the model sent only a product marker', () => {
    expect(parseGeneration('[[PRODUCTS:tee-2]]')).toEqual({
      text: '',
      handoff: false,
      productIds: ['tee-2'],
      usage: null,
