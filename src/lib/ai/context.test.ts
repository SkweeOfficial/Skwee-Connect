...
describe('buildConversationContext', () => {
  ...
  it('names the product cards a bot message carried', async () => {
    const out = await buildConversationContext(
      fakeDb([
        {
          sender_type: 'bot',
          content_text: 'Try these',
          interactive_payload: {
            kind: 'product_list',
            catalog_id: 'c1',
            sections: [
              { title: 'Party Dress', items: [] },
              { title: 'Linen Tee', items: [] },
            ],
          },
        },
      ]),
    ...
    expect(out).toEqual([
      {
        role: 'assistant',
        content: 'Try these\n[Sent product cards: Party Dress, Linen Tee]',
      },
    ])
  })
})
