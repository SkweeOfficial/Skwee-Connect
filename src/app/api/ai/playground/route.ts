...
export async function POST(request: Request) {
  try {
    ...
    const products = await loadProductContext(supabase, accountId, messages)
    ...
    const { text, handoff, productIds } = await generateReply({
      config,
      systemPrompt,
      messages,
    })
    // The card the bot would send on WhatsApp (null = text only).
    const productCards =
      products && !handoff
        ? buildProductMessagePayload({
            catalogId: products.catalogId,
            groups: resolvePickedGroups(products, productIds),
            bodyText: text,
          })
        : null
    return NextResponse.json({
      reply: text,
      handoff: handoff || (!text && !productCards),
