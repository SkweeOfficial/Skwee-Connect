            ...
            select: () => ({
              eq: () => ({
                in: () =>
                  Promise.resolve({
                    data: [
                      { retailer_id: 'dress__4-5Y', name: 'Party Dress', size: '4-5Y' },
                    ],
                    error: null,
                  }),
              }),
...
describe('inbound webhook: WhatsApp cart orders', () => {
  const orderMessage = {
    id: 'wamid.ORDER1',
    from: '15551230000',
    timestamp: '1700000000',
    ...
    order: {
      ...
      product_items: [
        { product_retailer_id: 'dress__4-5Y', quantity: 2, item_price: 1290, currency: 'INR' },
        { product_retailer_id: 'tee', quantity: '1', item_price: '790', currency: 'INR' },
      ],
    },
  }

  it('stores the cart as an order with named lines and a summary', async () => {
    await runWebhook(orderMessage)

    const row = h.state.upsertCalls[0].row
    expect(row).toMatchObject({
      content_type: 'order',
      content_text: '🛒 Order: 3 items · INR 3,370\nGift wrap please',
    })
    expect(row.order_payload).toEqual({
      catalog_id: '1234567890',
      text: 'Gift wrap please',
      items: [
        { retailer_id: 'dress__4-5Y', quantity: 2, item_price: 1290, currency: 'INR', name: 'Party Dress (4-5Y)' },
        { retailer_id: 'tee', quantity: 1, item_price: 790, currency: 'INR', name: null },
      ],
    })
  })

  it('hands the order to the public webhook and keeps the AI out of it', async () => {
    await runWebhook(orderMessage)
    ...
    expect(h.dispatchWebhookEvent).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        content_type: 'order',
        order: expect.objectContaining({ catalog_id: '1234567890' }),
      }),
    )
  })

  it('leaves order off the webhook payload for ordinary messages', async () => {
    await runWebhook()
    const payload = h.dispatchWebhookEvent.mock.calls[0][3] as Record<string, unknown>
