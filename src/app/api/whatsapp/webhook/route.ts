...
import { orderSummaryText } from '@/lib/commerce/catalog'
import type { OrderPayload } from '@/lib/commerce/types'
import {
...
interface WhatsAppMessage {
  ...
  /**
   * Set when the customer sends their WhatsApp cart after browsing our
   * catalog product cards — the line items they want to buy.
   */
  order?: MetaOrder
...
async function processMessage(
  ...
  const { contentText, mediaUrl, mediaType, interactiveReplyId, orderPayload: rawOrder } =
  ...
  // Name the ordered SKUs from the catalog cache (best-effort) so the
  // inbox and the public webhook show products, not bare ids.
  const orderPayload: OrderPayload | null = rawOrder
    ? await withCatalogNames(supabaseAdmin(), accountId, rawOrder)
    : null
    ...
    .upsert(
      {
        ...
        // Cart the customer sent from our catalog cards (migration 040);
  ...
  // Cart orders are skipped too: the customer has chosen what to buy, so
  // a human (or the store, via the public webhook) takes it from here.
  if (!flowConsumed && !interactiveReplyId && !orderPayload && inboundText.trim()) {
  ...
  await dispatchWebhookEvent(supabaseAdmin(), accountId, 'message.received', {
    ...
    content_type: contentType,
    text: contentText,
    // Only on `order` messages: the cart lines, for the store to turn
    // into a checkout.
...
async function parseMessageContent(
  ...
  /** For `order` messages: the cart the customer sent. Null otherwise. */
  ...
  switch (message.type) {
    ...
    case 'order': {
      // The customer sent their WhatsApp cart from our catalog product
      // cards. Keep the lines structured; contentText is a one-line
      // summary for the conversation list, with any note they typed.
      const order = parseOrderMessage(message.order)
      if (!order) return { ...empty, contentText: '[Order]' }
      const summary = orderSummaryText(order)
      return {
        ...empty,
        contentText: order.text ? `${summary}\n${order.text}` : summary,
        orderPayload: order,
      }
    }
