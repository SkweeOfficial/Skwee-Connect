...
The AI assistant can now **sell from WhatsApp**: when a customer asks
for a recommendation it answers with native WhatsApp product cards from
your Meta Commerce catalog, and the customer can add to cart and send the
order without leaving the chat.

> **Migration required:** apply `supabase/migrations/040_commerce_catalog.sql`
> (adds `whatsapp_config.catalog_id`, the `catalog_products` cache, and the
> `order` message type with `messages.order_payload`).

### Added

- **AI product cards.** Link your catalog under Settings → WhatsApp →
  Product Catalog and click **Sync now**. The auto-reply bot then sees the
  in-stock products that best match what the customer asked for and can
  send them as a single-product card or a product list (all sizes of a
  product in one section), with its reply as the card text. It can only
  send products that are in the synced catalog, and if Meta rejects a card
  it falls back to a plain text reply. The Playground shows the cards too.
- **WhatsApp cart orders.** When a customer sends their cart, the inbox
  shows the order (items, quantities, total, note) instead of
  "[Unsupported message type]". The AI stays out of it, and the
  `message.received` webhook now carries an `order` object so your store
  can turn it into a checkout. The public API's messages also include
  `order`.
- **`GET /api/commerce/cron`** keeps every linked catalog in sync (hourly
  is plenty). It uses the same `x-cron-secret` as the other cron endpoints.
