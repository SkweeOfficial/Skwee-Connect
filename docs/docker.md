...
- Nothing inside the container is scheduled. If you use automation
  ...
  (`AUTOMATION_CRON_SECRET`, see `.env.local.example`). If you use AI
  product cards, add `GET /api/commerce/cron` (hourly) the same way.
  All of them return 503 until that variable is set.
- The container healthcheck polls `GET /api/health`. For external
  monitoring (UptimeRobot) and the `?deep=1` Supabase check, see
